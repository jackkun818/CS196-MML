...
async def get_guidance(params: dict):
    ...
    # content = parse_docs(docs)
    content = retrieval_chain_guide(module_name=params.get("module_name"),question=params.get("question"))
    print("relevance_content={}".format(content))
    content = select_relevance_guide(module_name=params.get("module_name"),sim_docs=content)
    print("relevance_content={}".format(content))
