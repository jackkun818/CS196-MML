...
{
    public class MmseReportViewModel : BindableBase, IDialogResultable<object>, IDialogCommon<int?, int?, int?, string, string>
    {
        ...
        public MmseReportViewModel() { }
        public MmseReportViewModel(IRegionManager regionManager, IContainerProvider containerProvider, IEventAggregator eventAggregator, Crs_Db2Context db)
        {
            ...
            subjectItems = new ObservableCollection<SubjectItem>(items.Select(index => new SubjectItem { Name = $"topic{index}", StandardType = EvaluateStandardType.MMSE量表 }));
        }
